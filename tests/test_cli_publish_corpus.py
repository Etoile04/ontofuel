...
def test_cli_publish_corpus(tmp_path):
    rc = main(["publish-corpus", "--ontology", str(ONTO), "--corpus-root", str(tmp_path)])
    assert rc == 0
...
def test_cli_publish_corpus_idempotent(tmp_path):
    rc1 = main(["publish-corpus", "--ontology", str(ONTO), "--corpus-root", str(tmp_path)])
    assert rc1 == 0
    # second run is SKIPPED (no-op) -> still success (rc 0)
    rc2 = main(["publish-corpus", "--ontology", str(ONTO), "--corpus-root", str(tmp_path)])
    assert rc2 == 0
